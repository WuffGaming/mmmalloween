...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'moonrise' | 'ronman' | 'little-man-halloween' | '($@($%()@_@#%@}{{{{':
				...
				var bg:FlxSprite;
				var home:FlxSprite;
				bg = new FlxSprite(-100,10).loadGraphic(Paths.image('halloween boo/house/bg'));
				home = new FlxSprite(-130, 0).loadGraphic(Paths.image('halloween boo/house/house'));
				bg.updateHitbox();
				bg.active = false;
				bg.antialiasing = true;
	...
	function repositionDad() {
		...
		FlxG.camera.follow(camFollow, LOCKON, camFollowLerp * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
		...
		scoreTxt = new FlxText(FlxG.width / 2 - 235, healthBarBG.y + 50, FlxG.width, "", 20);
		...
		{
			case 'hellstage':
				insert(100000, bobmadshake); // We don't want this to be behind anything.
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (!isStoryMode)
			{
				...
				if (SONG.song.toLowerCase() == '($@($%()@_@#%@}{{{{')
				{
					babyArrow.y += 10;
					new FlxTimer().start(2, function(tmr:FlxTimer)
					{
						FlxTween.tween(babyArrow, {alpha: 1}, 10, {ease: FlxEase.linear, startDelay: 0.5});
					});	
				}
				else
				{
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			Discord.changePresence("PAUSED on " + SONG.song + " (" + storyDifficultyText + ") " + generateRanking(), "Acc: " + truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses);
			#end
	...
	override public function update(elapsed:Float)
	{
		...
		if (!offsetTesting)
		{
			...
			{
				scoreTxt.text = (FlxG.save.data.npsDisplay ? "NPS: " + nps + " | " : "") + "Score:" + (Conductor.safeFrames != 10 ? songScore + " (" + songScoreDef + ")" : "" + songScore) + " | Misses:" + misses + " | Accuracy:" + truncateFloat(accuracy, 2) + "% | " + generateRanking();
			}
			else
			{
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			if(FlxG.save.data.cameraRating)
			{
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		{
			var canMash = true; // Can the player even mash?
			if (SONG.song.toLowerCase() == 'run') canMash = false;
	...
	override function stepHit()
	{
		...
		// Updating Discord Rich Presence (with Time Left)
		Discord.changePresence(detailsText + " " + SONG.song + " (" + storyDifficultyText + ") " + generateRanking(), "Acc: " + truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses);
		#end
	...
	override function beatHit()
	{
		...
		if (curSong == 'Ron')
		{
			...
			{
				FlxG.camera.follow(dad, LOCKON, camFollowLerp * (30 / (cast (Lib.current.getChildAt(0), Main)).getFPS()));
				FlxTween.tween(FlxG.camera, {zoom: 1.5}, 0.4, {ease: FlxEase.expoOut,});
				dad.playAnim('cheer', true);
			}
			else
